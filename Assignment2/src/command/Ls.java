...
/**
 * Ls is a Command where it can print all the folders and files under current working directory.
 * 
 * @author Chongmin Bai
 */
public class Ls extends Command {
  ...
  public void execute(ArrayList<Object> param) {
    ...
    if (input.contains("-R")) {
      if (numOfArg == 2) {
        ...
        while (flag < numOfArg) {
          if (FileSystemManipulation
              .findFileSystemNode(userInput[flag]) != null) {
            printR((Directory) FileSystemManipulation
                .findFileSystemNode(userInput[flag]));
            flag++;
          } else
      ...
      if (numOfArg == 1)
        ...
        while (count < numOfArg) {
          printFileAndDir(workingDir, userInput[count++]);
        }
      }
    }
  }
  ...
  private void printFileAndDir(Directory workingDir, String path) {
    ...
    if (path.isEmpty() == true)
      ...
      FileSystemNode subNode =
          FileSystemManipulation.findSubNode(workingDir, path);
  ...
  private void printFilesAndDirectories(Directory workingDir) {
    ...
    ArrayList<String> sortedList =
        new ArrayList<String>(workingDir.getListOfFileSystemNodes().keySet());
